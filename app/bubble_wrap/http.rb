##
# from https://github.com/mattetti/BubbleWrap/blob/master/lib/http.rb
##
module BubbleWrap
  ...
  module HTTP
    ...
    class Query
      ...
      ##
      # This solved the http basic auth bug for github
      ##
      def connection(connection, willSendRequest:request, redirectResponse:response)
        #p "willsendRequest:redirectResponse"
        #p "sending #{@credentials.inspect}"
        #updatedRequest = request.mutableCopy
        #updatedRequest.setValue(@credentials.inspect, forHTTPHeaderField:'Authorization')
        #updatedRequest
        puts "HTTP redirected #{request.description}"
        new_request = request.mutableCopy
        #@request.setAllHTTPHeaderFields(@headers)
        #p "headers from @headers: #{@headers}"
        #p "headers from new_request: #{new_request.allHTTPHeaderFields.inspect}"
        #@request.setAllHTTPHeaderFields(@headers) if @headers
        #p @request.allHTTPHeaderFields.inspect
        @connection.cancel
      ...
      def connection(connection, willSendRequestForAuthenticationChallenge:challenge)
        ...
        p "Challenge: #{challenge.inspect}"
        if (challenge.previousFailureCount < 10)
          new_credential = NSURLCredential.credentialWithUser(credentials[:username],
          password:credentials[:password], persistence:NSURLCredentialPersistenceForSession)
